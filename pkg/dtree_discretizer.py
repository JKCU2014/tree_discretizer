...
class BaseDTDiscretizer(six.with_metaclass(ABCMeta,
    ...
    def __init__(self,
        ...
        self.combine_name_nodes = combine_name_nodes
    ...
    def fit(self, X, y):
        ...
        if self.combine_name_nodes:
            self._gen_column_names_with_combination(decimal_form)
        else:
            self._gen_column_names_without_combination(decimal_form)
    ...
    def _gen_column_names_with_combination(self, decimal_form):
        """helper函数，生成交叉项名称时会组合使用相同特征的节点

        :param decimal_form: str
        :return: None, Modified函数
        """
        i = 0
        for new_fea_path in self.paths_:
            name_tmp = {}
            for fea_thre, comp in zip(new_fea_path[0], new_fea_path[1]):
                ...
                if not fea_tmp in name_tmp.keys():
                    ...
                    name_tmp[fea_tmp][comp] = min(name_tmp[fea_tmp][comp], thre_tmp) if comp == '<=' else max(
                        name_tmp[fea_tmp][comp], thre_tmp)
                else:
                    name_tmp[fea_tmp][comp] = thre_tmp
    ...
    def _gen_column_names_without_combination(self, decimal_form):
        """helper函数，生成交叉项名称时不会组合使用相同特征的节点
        ...
        for new_fea_path in self.paths_:
            name_tmp = []
            for fea_thre, comp in zip(new_fea_path[0], new_fea_path[1]):
                name_tmp.append(self.columns[fea_thre[0]] + comp + decimal_form % fea_thre[1])
            self.tree_inter_feature_names_.append(self.sep.join(name_tmp))
    ...
    def _construct_tree(self, tree_features, tree_threshold):
        ...
        # 左子结点; 当该项==self.v_leaf_时,后一项是前面某项的右子节点,通过nodes_stack.pop()
        # 找到该项
        curr_node = root
...
class ClassificationDTDiscretizer(BaseDTDiscretizer):
    """基于决策树的独热编码型交叉项生成器,可返回新特征及特征名称
        ...
        inters_only: bool (default=False)
            是否只使用新生成的离散型交叉项
        combine_name_nodes: bool
            生成新特征名称时是否组合使用同一特征的节点条件
        tree: sklearn.tree
    ...
    def __init__(self,
        ...
        super(ClassificationDTDiscretizer, self).__init__(
            ...
            combine_name_nodes=combine_name_nodes,
            classification=True,
            tree=tree,
            tree_=tree_,
            max_depth=max_depth,
...
class GBDTDiscretizer(BaseEstimator,
    ...
    def __init__(self,
                 ...
                 classification=True,
                 **kwargs):
        """创建一个GBDTDiscretizer对象

        :param columns: array-like shape = [n_features]
            源数据集特征名称
        :param n_decimals: int (default=3)
            生成的交叉项特征名称中threshold小数点后保留位数
        :param v_convert_functions: dict [特征名称: 转化函数] or None
            生成结点名称时,如果特征数据在传入前经过函数转化可在此处提供反函数,使得新生成特征
            名称的阀值为原数值
        :param sep: str (default=', ')
            在对新生成的交叉项进行命名时,discretizer对各结点条件进行组合使用的分隔符号
        :param inters_only: bool (default=False)
            是否只使用新生成的离散型交叉项
        :param remove_dup_features: bool
            是否删除具有相同结点条件的特征
        ...
        self.sep = sep
        self.inters_only = inters_only
        self.remove_dup_features = remove_dup_features
        self.classification = classification
        self.kwargs = kwargs
    ...
    def fit(self, X, y):
        ...
        self.tree_discretizers_ = [RegressionDTDiscretizer(columns=self.columns,
                                                      ...
                                                      sep=self.sep,
                                                      inters_only=True,
                                                      combine_name_nodes=self.remove_dup_features,
                                                      tree_=tree_).fit(X, y)
                              for tree_ in tree_s]
